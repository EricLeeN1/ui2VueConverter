...
**验收标准**
- [ ] 输入一个 CSS 缺少 3 个 `}` 的代码，自动修复后能正常通过 Prettier 格式化
- [ ] 无法自动修复时，输出类似：`错误：.stamp { (第955行) 未闭合，已追踪到嵌套深度5层`
...
**验收标准**
- [ ] 生成代码中不出现 `import { ref } from 'vue'`（项目有 auto-import 时）
- [ ] 生成代码中不出现 `import { showToast } from 'vant'`（项目有 VantResolver 时）
- [ ] 自定义导入（如 `import toast from '@/utils/toast'`）保留不变
